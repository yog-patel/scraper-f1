...
async function ensureConnected() {
  if (!client || !clientConnected || client._ending || client._connecting) {
    await connectClient();
  } else {
    // Check for "not queryable" state (pg internals)
    if (client._queryable === false || client._ending) {
      await connectClient();
    }
  }
}
...
async function withDbRetry(fn, maxRetries = 2) {
  ...
  while (attempt < maxRetries) {
    try {
      ...
      attempt++;
      // Only retry for connection errors, not for all errors
      if (
        err.message &&
        (
        ...
        console.warn(`DB connection lost, reconnecting and retrying (${attempt}/${maxRetries})...`, err && err.message);
        await connectClient();
...
// Function to insert chapter data (gentle, small batches, slow)
async function insertChapters(novelId, chapters) {
  return withDbRetry(async () => {
    ...
    const batchSize = 5; // Very small batch to minimize lock time and connection usage
    ...
    for (let batchStart = 0; batchStart < chapters.length; batchStart += batchSize) {
      ...
      for (let i = 0; i < batch.length; i++) {
        ...
        const chapterNumber = latestChapterNumber + newChaptersCount + 1;
        try {
          await ensureConnected();
          ...
          // Slow down to avoid overwhelming the DB
          await new Promise(res => setTimeout(res, 1200));
        } catch (error) {
          console.error(`Error inserting chapter ${chapterNumber}:`, error);
          // Stop inserting further chapters if one fails
